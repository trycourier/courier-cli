...
var automationsRunsListSteps = cli.Command{
	...
	Action:          handleAutomationsRunsListSteps,
	HideHelpCommand: true,
}

func handleAutomationsRunsList(ctx context.Context, cmd *cli.Command) error {
	...
	params := courier.AutomationRunListParams{}

	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Automations.Runs.List(ctx, params, options...)
	if err != nil {
		return err
	}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Title:          "automations:runs list",
		Transform:      transform,
	})
}

func handleAutomationsRunsListSteps(ctx context.Context, cmd *cli.Command) error {
	client := courier.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Automations.Runs.ListSteps(ctx, cmd.Value("id").(string), options...)
	if err != nil {
		return err
	}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "automations:runs list-steps",
		Transform:      transform,
	})
}
